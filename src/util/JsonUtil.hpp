...
template <> struct adl_serializer<void *> {
    static void to_json(json &j, const void *ptr) { j = nullptr; }
...
template <typename T> T fromJson(const std::string &value) {
...
template <typename T> nlohmann::json toJsonTree(const T &obj) { return obj; }
